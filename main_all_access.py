...
def _get_token() -> str:
    token = (os.getenv("8916112195:AAE1W-EkC0fpCK92uXjsl9FegFfIb4c2X3s") or "8916112195:AAE1W-EkC0fpCK92uXjsl9FegFfIb4c2X3s").strip()
    if token:
        return token
...
def deal_card_html(deal: dict, viewer_id: int) -> str:
    ...
    if viewer_id and viewer_id == seller_id and status in ("paid", "joined"):
        hint = (
            ...
            f"Передайте подарок менеджеру <code>@FunpayDealsManager</code>, "
        ...
        hint = (
            ...
            f"из сделки действительно получен юзером <code>@FunpayDealsManager</code>.\n"
...
async def on_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if step not in ("ban", "unban", "give_balance", "give_deals"):
        return
    raw = msg.text.strip()
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("vestaidteam", cmd_admin))
